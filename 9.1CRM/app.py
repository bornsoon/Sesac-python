...
def userDetail(id):
    ...
    order_query = "SELECT i.item, o.orderAt, s.name FROM orders o JOIN stores s ON o.storeId=s.Id JOIN orderItems oi ON o.id=oi.orderId JOIN items i ON oi.itemId=i.id WHERE o.userId = ? ORDER BY o.orderAt"
    orders = db.get_query(order_query, (id,))

    store_query = "SELECT s.name FROM orders o INNER JOIN stores s ON o.storeId=s.Id WHERE o.userId = ? GROUP BY s.Id ORDER BY count(s.Id) DESC LIMIT 5"
    topStores = db.get_query(store_query, (id,))

    item_query = "SELECT i.item FROM items i INNER JOIN orderItems oi ON i.Id=oi.itemId INNER JOIN orders o ON oi.orderId=o.id WHERE o.userId = ? GROUP BY i.Id ORDER BY count(i.id) DESC LIMIT 5"
